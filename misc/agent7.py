...
async def main():
    ...
    options = ClaudeAgentOptions(
        ...
        # auto-approve edits so it runs unattended
        extra_args={"replay-user-messages": None},
        # required to receive checkpoint UUIDs
    )
    async with ClaudeSDKClient(options) as client:
        await client.query(
            "Add architecture comments throughout the cvss code."
        )
        async for message in client.receive_response():
            if (
                ...
                print(f"Stored Checkpoint ID: {checkpoint_id}")
    ...
    if checkpoint_id and session_id:
        async with ClaudeSDKClient(
            ...
            async for message in client.receive_response():
                print(f"Rewinding to Checkpoint ID: {checkpoint_id}")
