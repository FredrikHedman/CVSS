* <2014-03-08 Sat>
** PEP8 checking
** Different scores need refactoring
   Use DRY on vulnerability.py and cvss.py to factor out data related
   to scores.  Currently quite a lot of repeated data.

   For example see BASE_SCORE and base_metrics()

** Refactor cvss.py 
Should contain the logic for the command line syntax.  The rest should
be factored out into other files.
...
* <2014-03-07 Fri>
** Should transform this to a packgage with cvss.py
as the main program.  The examples should no longer need soft links to
run. and it should be possible to package cvss.py as a stand alone program.
** test_vulnerability.py och vulnerability.md should move to tests
** README.md needs to be updated to refelct new structure
** Scripts in tests need to be refactored
In particular the should be made more location independent.

** General clean-up and restructuring
** Commit the changes that were made in november.
* 2013-11-14
