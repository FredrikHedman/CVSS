...
*** Refactor cli (cli2)
    Simplify!  Not selected.  No output.!  Not quite working when when
    no interactive set.
*** Start using cmd
...
** Parse vectors and calculate scores
   Score ore error.  Use default values when no available.
** Normalize output
   Print scores and vectors.
** rst output
   Generate verbose output in a RST format.
** plain text output
** parse a vector and return a score
