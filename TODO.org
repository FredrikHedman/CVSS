* <2026-05-08 Fri> Findings after override/@TypedDict/annotation cleanup [2/4]
  ...
  4) [ ] **`CommonVulnerabilityScore` hardcoded metric key lists** —
...
* <2026-05-08 Fri> Findings from partition/ValueError/dataclass/print cleanup [3/4]
  ...
  2) [X] **`except Exception` → specific exception types in `cvss.py`** —
     ...
     Narrowing to `except (InvalidBaseVectorError, ValueError)` prevents silent
     swallowing of unexpected errors.
  3) [ ] **`CommonVulnerabilityScore` hardcoded metric key lists** —
     `base_metrics()`, `base_vector`, `temporal_metrics()`, `temporal_vector`,
